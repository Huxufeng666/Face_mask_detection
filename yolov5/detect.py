...
@torch.no_grad()
def run(weights_v5=ROOT / '',  # model.pt path(s)
    ...
    model = DetectMultiBackend(weights_v5, device=device, dnn=dnn)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_txt:  # Write to file
                        ...
                        line = (*xywh, conf,cls) if save_conf else (cls, *xywh)  # label format
                       
                    # BBox.append([(*xywh,np.array((conf.cpu().detach().numpy()),dtype=float),(cls.cpu().detach().numpy()))])       
                    BBox.append([(*xywh,conf.cpu().detach().numpy(),cls.cpu().detach().numpy())])       
    ...
    if update:
        strip_optimizer(weights_v5)  # update model (to fix SourceChangeWarning)

    return  det[:, :4]

def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights_v5', nargs='+', type=str, default=ROOT / '', help='model path(s)')
